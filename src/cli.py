            ...
            if board.turn == chess.WHITE:
                ...
                while True:
                    ...
                    try:
                        ...
                        best_eval = engine.analyse(board, chess.engine.Limit(depth=8))["score"].relative.score(
                            mate_score=10000
                        )
                        board.pop()
                        ...
                        board.push(move)
                        played_eval = engine.analyse(board, chess.engine.Limit(depth=8))["score"].relative.score(
                            mate_score=10000
                        )
                        board.pop()
...
def run_ui(args):
    ...
    subprocess.run([sys.executable, "-m", "streamlit", "run", str(ui_path), "--server.port", str(args.port)])
...
Examples:
  ...
  python -m src.cli ui --port 8501            # Start web UI
    ...
    play_parser.add_argument(
        "--depth", "-d", type=int, default=4, dest="initial_depth", help="Initial bot depth (1-8, default: 4)"
    )
    play_parser.set_defaults(func=play_game)

    # Analyze command
    analyze_parser = subparsers.add_parser("analyze", help="Analyze a chess position")
    analyze_parser.add_argument("--fen", "-f", type=str, required=True, help="FEN string of the position to analyze")
    analyze_parser.add_argument("--depth", "-d", type=int, default=12, help="Analysis depth (default: 12)")
    analyze_parser.add_argument("--lines", "-l", type=int, default=1, help="Number of top moves to show (default: 1)")
    ...
    elo_parser.add_argument("--acpl", "-a", type=float, required=True, help="Average Centipawn Loss value")
    ...
    server_parser.add_argument("--port", "-p", type=int, default=8000, help="Port to bind to (default: 8000)")
    server_parser.add_argument("--reload", "-r", action="store_true", help="Enable auto-reload for development")
    server_parser.set_defaults(func=run_server)

    # UI command
    ui_parser = subparsers.add_parser("ui", help="Start the Streamlit UI")
    ui_parser.add_argument("--port", "-p", type=int, default=8501, help="Port to bind to (default: 8501)")
